...
public class ManagerTHumano {
	...
	public List<DTOThmEmpleado> findAllDTOThmEmpleado() {
		List<DTOThmEmpleado> empleadosResponse = new ArrayList<DTOThmEmpleado>();
		String field = "";
		field = "segUsuario.activo";
		String where = "o.".concat(field).concat("=").concat("true");// PSQL for active empleados
		List<ThmEmpleado> empleados = mDAO.findWhere(ThmEmpleado.class, where, "idThmEmpleado");
		field = "thmEmpleado.idThmEmpleado";
		for (ThmEmpleado emp : empleados) {
			DTOThmEmpleado dtoEmp = new DTOThmEmpleado(emp.getSegUsuario().getNombres(),
					emp.getSegUsuario().getApellidos(), emp.getSegUsuario().getCorreo(),
					emp.getThmCargo().getNombreCargo(), emp.getThmCargo().getRemuneracionMensual());
			where = "o.".concat(field).concat("=").concat("'").concat(emp.getIdThmEmpleado().toString()).concat("'");
			List<ThmRolCabecera> roles = this.mDAO.findWhere(ThmRolCabecera.class, where, "periodoRol");
			List<DTOThmRolPago> rolesDTO = new ArrayList<>();
			for (ThmRolCabecera rol : roles) {
				rolesDTO.add(new DTOThmRolPago(rol.getPeriodoRol(), rol.getSubtotalIngresos(),
						rol.getSubtotalIngresosAdicionales(), rol.getSubtotalEgresos(), rol.getTotal()));
			}
			dtoEmp.setRolesPago(rolesDTO);
			empleadosResponse.add(dtoEmp);
		}
		return empleadosResponse;
	}

	@SuppressWarnings("unchecked")
	public DTOThmEmpleado findDTOThmEmpleadoByIdSegUsuario(Integer id) {
		String field = "segUsuario.idSegUsuario";
		String where = "o.".concat(field).concat("=").concat("'").concat(id.toString()).concat("'").concat(" AND ")
				.concat("segUsuario.activo").concat("=true");
		List<ThmEmpleado> empleados = this.mDAO.findWhere(ThmEmpleado.class, where, "idThmEmpleado");
		if (empleados.size() != 1) {
			return null;
		}
		ThmEmpleado emp = empleados.get(0);
		field = "thmEmpleado.idThmEmpleado";
		where = "o.".concat(field).concat("=").concat("'").concat(emp.getIdThmEmpleado().toString()).concat("'");
		List<ThmRolCabecera> roles = this.mDAO.findWhere(ThmRolCabecera.class, where, "periodoRol");
		List<DTOThmRolPago> rolesDTO = new ArrayList<>();
		...
		DTOThmEmpleado dtoEmp = new DTOThmEmpleado(emp.getSegUsuario().getNombres(), emp.getSegUsuario().getApellidos(),
				emp.getSegUsuario().getCorreo(), emp.getThmCargo().getNombreCargo(),
				emp.getThmCargo().getRemuneracionMensual(), rolesDTO);
		return dtoEmp;
	}
	...
	public boolean isActiveUser(Integer id) {
		...
		try {
			user = (SegUsuario) this.mDAO.findById(SegUsuario.class, id);
			response = user.getActivo();
		} catch (Exception e) {
			response = false;
		}
		return response;
	...
	public void insertarCargo(ThmCargo cargo) throws Exception {
		this.mDAO.insertar(cargo);
	}

	public void eliminarCargo(Integer id) throws Exception {
		this.mDAO.eliminar(ThmCargo.class, id);
	}

	public void actualizarCargo(ThmCargo cargo) throws Exception {
		this.mDAO.actualizar(cargo);
	}

	public Integer getSequence(String name) {

		try {
			return this.mDAO.obtenerSecuenciaPostgresql(name).intValue();
		} catch (Exception e) {
